...
class BioStimulantsScreen extends StatefulWidget {
  const BioStimulantsScreen({super.key});

  @override
  State<BioStimulantsScreen> createState() => _BioStimulantsScreenState();
}

class _BioStimulantsScreenState extends State<BioStimulantsScreen> {
  ...
  // List of allowed product image assets (for placeholder only)
  ...
  // List of Bio Stimulant products, fetched from ProductService
  List<Product> bioStimulantProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchBioStimulantProducts();
  }

  void _fetchBioStimulantProducts() {
    // Using ProductService to get products specific to 'Bio Stimulants'
    setState(() {
      bioStimulantProducts = ProductService.getProductsByCategory('Bio Stimulants');
    });
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildHeaderSection('Our Bio Stimulant Products'),
              const SizedBox(height: 12),
              _buildProductGrid(bioStimulantProducts), // Use fetched products
              const SizedBox(height: 12),
              _buildViewAllButton(),
              const SizedBox(height: 28),
              _buildHeaderSection('New On Bio Stimulants'),
              const SizedBox(height: 12),
              _buildProductGrid(bioStimulantProducts.take(2).toList()), // Example: subset for "New On"
              const SizedBox(height: 12),
              _buildViewAllButton(),
  ...
  Widget _buildDealGrid() {
    ...
    // If you want actual product deals, you'd fetch them similarly to `bioStimulantProducts`
    // and then map them to this UI, ensuring they have 'old' and 'new' price fields.
    List<Map<String, String>> deals = [
      {'name': 'BOOSTALL', 'image': _getImageAsset(0), 'old': '1000', 'new': '750'},
      {'name': 'VITALIZER', 'image': _getImageAsset(1), 'old': '1300', 'new': '999'},
      {'name': 'GREENUP', 'image': _getImageAsset(2), 'old': '1600', 'new': '1250'},
      {'name': 'NUTRIBOOST', 'image': _getImageAsset(0), 'old': '1100', 'new': '850'},
    ];
