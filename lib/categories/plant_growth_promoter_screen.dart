...
class PlantGrowthPromoterScreen extends StatefulWidget {
  const PlantGrowthPromoterScreen({super.key});

  @override
  State<PlantGrowthPromoterScreen> createState() => _PlantGrowthPromoterScreenState();
}

class _PlantGrowthPromoterScreenState extends State<PlantGrowthPromoterScreen> {
  ...
  // List of Plant Growth Promoter products, fetched from ProductService
  List<Product> plantGrowthPromoterProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchPlantGrowthPromoterProducts();
  }

  void _fetchPlantGrowthPromoterProducts() {
    // Using ProductService to get products specific to 'Plant Growth Promoter'
    setState(() {
      plantGrowthPromoterProducts = ProductService.getProductsByCategory('Plant Growth Promoter');
    });
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildHeaderSection('Our Plant Growth Promoter Products'),
              const SizedBox(height: 12),
              _buildProductGrid(plantGrowthPromoterProducts), // Use fetched products
              const SizedBox(height: 12),
              _buildViewAllButton(),
              const SizedBox(height: 28),
              _buildHeaderSection('New On Plant Growth Promoters'),
              const SizedBox(height: 12),
              _buildProductGrid(plantGrowthPromoterProducts.take(2).toList()), // Example: subset for "New On"
              const SizedBox(height: 12),
              _buildViewAllButton(),
  ...
  Widget _buildDealGrid() {
    ...
    // If you want actual product deals, you'd fetch them similarly to `plantGrowthPromoterProducts`
    // and then map them to this UI, ensuring they have 'old' and 'new' price fields.
    List<Map<String, String>> deals = [
      {'name': 'GROWFAST', 'image': _getImageAsset(0), 'old': '1200', 'new': '950'},
      {'name': 'FLOURISH', 'image': _getImageAsset(1), 'old': '1500', 'new': '1100'},
      {'name': 'YIELDMAX', 'image': _getImageAsset(2), 'old': '1800', 'new': '1400'},
      {'name': 'BOOSTUP', 'image': _getImageAsset(0), 'old': '1300', 'new': '1050'},
    ];
