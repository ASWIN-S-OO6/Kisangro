...
class InsecticideScreen extends StatefulWidget {
  const InsecticideScreen({super.key});

  @override
  State<InsecticideScreen> createState() => _InsecticideScreenState();
}

class _InsecticideScreenState extends State<InsecticideScreen> {
  ...
  // List of Insecticide products, fetched from ProductService
  List<Product> insecticideProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchInsecticideProducts();
  }

  void _fetchInsecticideProducts() {
    // Using ProductService to get products specific to 'Insecticide'
    setState(() {
      insecticideProducts = ProductService.getProductsByCategory('Insecticide');
    });
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildHeaderSection('Our Insecticide Products'),
              const SizedBox(height: 12),
              _buildProductGrid(insecticideProducts), // Use fetched products
              const SizedBox(height: 12),
              _buildViewAllButton(),
              const SizedBox(height: 28),
              _buildHeaderSection('New On Insecticides'),
              const SizedBox(height: 12),
              _buildProductGrid(insecticideProducts.take(2).toList()), // Example: subset for "New On"
              const SizedBox(height: 12),
              _buildViewAllButton(),
  ...
  Widget _buildDealGrid() {
    ...
    // If you want actual product deals, you'd fetch them similarly to `insecticideProducts`
    // and then map them to this UI, ensuring they have 'old' and 'new' price fields.
    List<Map<String, String>> deals = [
      {'name': 'INSECTKILL', 'image': _getImageAsset(0), 'old': '1800', 'new': '1450'},
      {'name': 'BUGGONE', 'image': _getImageAsset(1), 'old': '1500', 'new': '900'},
      {'name': 'PESTGUARD', 'image': _getImageAsset(2), 'old': '2200', 'new': '1700'},
      {'name': 'INSECTSHIELD', 'image': _getImageAsset(0), 'old': '1900', 'new': '1300'},
    ];
