...
class HerbicideScreen extends StatefulWidget {
  const HerbicideScreen({super.key});

  @override
  State<HerbicideScreen> createState() => _HerbicideScreenState();
}

class _HerbicideScreenState extends State<HerbicideScreen> {
  ...
  // List of Herbicide products, fetched from ProductService
  List<Product> herbicideProducts = [];

  @override
  void initState() {
    super.initState();
    _fetchHerbicideProducts();
  }

  void _fetchHerbicideProducts() {
    // Using ProductService to get products specific to 'Herbicide'
    setState(() {
      herbicideProducts = ProductService.getProductsByCategory('Herbicide');
    });
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildHeaderSection('Our Herbicide Products'),
              const SizedBox(height: 12),
              _buildProductGrid(herbicideProducts), // Use fetched products
              const SizedBox(height: 12),
              _buildViewAllButton(),
              const SizedBox(height: 28),
              _buildHeaderSection('New On Herbicides'),
              const SizedBox(height: 12),
              _buildProductGrid(herbicideProducts.take(2).toList()), // Example: subset for "New On"
              const SizedBox(height: 12),
              _buildViewAllButton(),
  ...
  Widget _buildDealGrid() {
    ...
    // If you want actual product deals, you'd fetch them similarly to `herbicideProducts`
    // and then map them to this UI, ensuring they have 'old' and 'new' price fields.
    List<Map<String, String>> deals = [
      {'name': 'WEEDAWAY', 'image': _getImageAsset(0), 'old': '2500', 'new': '2000'},
      {'name': 'CLEARFIELD', 'image': _getImageAsset(1), 'old': '2100', 'new': '1500'},
      {'name': 'GREENKILL', 'image': _getImageAsset(2), 'old': '2800', 'new': '2200'},
      {'name': 'HERBIFREE', 'image': _getImageAsset(0), 'old': '2000', 'new': '1400'},
    ];
